        ...
        <table class="table">
            ...
            <tbody>
                <?php
                    ...
                    $contact = mysqli_query($db,"SELECT * FROM contacts ");
                    if ($contact->num_rows > 0) {
                        // output data of each row
                        while($row = $contact->fetch_assoc()) {
                            echo '<tr>';
                                echo '<th scope="row">'.$row["Number"].'</td>';
                                echo '<td scope="row">'.$row["title"].'</td>';
                                echo '<td scope="row">'.$row["email"].'</td>';
                                echo '<td scope="row">'.$row["message"].'</td>';
                                echo '<td scope="row"><a href="delete.php?Number='.$row["Number"].'" class="btn btn-danger">Delete</a></td>';
                            echo '</tr>';
                        }
                    } else {
                        echo "0 results";
