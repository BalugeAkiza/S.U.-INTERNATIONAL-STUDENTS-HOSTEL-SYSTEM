        ...
        <table class="table">
            ...
            <tbody>
                <?php
                    ...
                    $hostel = mysqli_query($db,"SELECT * FROM international_student");
                    if ($hostel->num_rows > 0) {
                        ...
                        while($row = $hostel->fetch_assoc()) {
                            echo '<tr>';
                                ...
                                echo '<td scope="row">'.$row["country_of_origin"].'</td>';
                                echo '<td scope="row">'.$row["email"].'</td>';
                                echo '<td scope="row">'.$row["phone_number"].'</td>';
                                echo '<td scope="row"><a href="Edit_user.php?student_id='.$row["id"].'" class="btn btn-success">Edit</a></td>';
                                echo '<td scope="row"><a href="delete.php?student_id='.$row["id"].'" class="btn btn-danger">Delete</a></td>';
                            echo '</tr>';
                        }
                    } else {
                        echo "0 results";
