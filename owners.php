        ...
        <table class="table">
            ...
            <tbody>
                <?php
                    ...
                    $hostel = mysqli_query($db,"SELECT * FROM hostel_owner");
                    if ($hostel->num_rows > 0) {
                        // output data of each row
                        while($row = $hostel->fetch_assoc()) {
                            echo '<tr>';
                                echo '<th scope="row">'.$row["owner_id"].'</td>';
                                ...
                                echo '<td scope="row"><a href="Edit_owner.php?owner_id='.$row["owner_id"].'" class="btn btn-success">Edit</a></td>';
                                echo '<td scope="row"><a href="delete.php?owner_id='.$row["owner_id"].'" class="btn btn-danger">Delete</a></td>';
                            echo '</tr>';
                        }
                    } else {
                        echo "0 results";
